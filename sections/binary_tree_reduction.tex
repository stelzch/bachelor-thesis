...
\Cref{fig:distributed_binary_tree} shows the distribution of nine elements are across three \glspl{pe}.
\begin{figure}[H]
\centering
\begin{tikzpicture}
...
Reductions $(0) \reduce (1)$, $(4) \reduce (5)$ and $(6) \reduce (7)$ can all be performed in parallel, since there exist no data dependencies between them.
...
In our example in \Cref{fig:distributed_binary_tree}, the nodes with coordinates $(2,1)$ and $(4,2)$ labeled A and B, respectively, are examples of \gls{pe}-intersecting nodes.
...
The target implementation utilizes the \gls{mpi} to communicate between different \glspl{pe} and since the messages are small (one double precision floating-point value occupies 8 bytes, or 64 bits), the communication overhead is dominated by the total number of messages that \glspl{pe} exchange.
...
For the $n_i^\textrm{lower}$-distribution, the global minima is attained at $N = 2^i * p$, where $i \in \mathbb{N}$.
In this case, each \gls{pe} holds an independent subtree of size $2^i$ whose root node is part of a \gls{pe}-intersecting node.
For $N+1$, all \gls{pe}-boundaries are shifted one step to the right because of the additional element on the root \gls{pe}.
...
\Cref{fig:messageCount256Lower} displays \Cref{eq:upperBoundLowerDistribution} in orange color.
Note that for the $n_i^\textrm{upper}$ distribution in \Cref{fig:messageCount256Upper}, the corresponding spikes are much less pronounced, since the subtree shifting described above does not occur when only adding one element.
