...
For any given $x$-coordinate $x > 0$, the \textbf{maximum $y$-coordinate} $\max_y(x)$ is equal to number of trailing zeros of $x$, i.e.\ the zero-indexed position of the least-significant bit set in $x$.
...
\KwData{\gls{pe}-rank $rank$, $n_{rank}$ summands}
...
\Cref{algo:SummationAlgo} shows to procedure that each \gls{pe} follows.
...
While the implementation used in \Cref{ch:Experiments} follows \Cref{algo:SummationAlgo} in principle, it includes multiple optimizations concerning both the calculation and the communication of results.
Message Buffering (\Cref{sec:MessageBuffering}) allows to reduce the number of messages sent across the cluster and therefore reduces communication times.
The number of messages further decreases if the program is in control over the data distribution (\Cref{sec:DataDistribution}).
Vectorizing the additions (\Cref{sec:Vectorization}) yielded the largest performance gain of all optimizations presented in this section.
...
While this model does not take any data dependencies between subtotals into account, it balances the performance gain achieved by parallelization (represented by a low maximum on the right side of \Cref{eq:distributionScore}) against the communication overhead caused by binary tree fragmentation. 
...
\Cref{eq:LargestChildIndex} is an expression that calculates the $largest\_child\_index$ with bitwise operations.
...
$messageCount = 0$\;
\ForEach{neighbouring elements $(startIndex, endIndex)$ of $startIndices$}{
	$index = startIndex$\;
	\While{$index < endIndex$}{
		$messageCount = messageCount + 1$\;
		$index = largest\_child\_index(index) + 1$\;
	}
}
\KwRet{$messageCount$}
\end{algorithm}
...
Distributing the data evenly, as presented in \Cref{eq:upperDistribution}, ensures maximal parallelization of the computational effort, since the maximum difference in the workload of \glspl{pe} is $1$.
